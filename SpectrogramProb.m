...
function [rEnergyAll,f,t]=SpectrogramProb(X)
folderName ='../data1/';
tag = 'test/';
...
% rEnergy_sub=A(f_cal<200,20:(end-20));%-B;%-rEnergy_base;
[rEnergy_sub,f,t]=cwtft({X(:,trial,ch),1},'wavelet','morl' );
%spectrogram(squeeze(X(:,trial,ch)),32,16,64,'center',Fs);
%cwtft({sig2,dt},'wavelet','morl' )
...
rEnergy_sample(trialNum,:,:)=10*log10(abs(rEnergy_sub));
f=f(f>0);
end
